...
namespace DesktopPet
{
    ...
    class ContextMenus : IDisposable
    {
        ...
        static ToolStripMenuItem newSheepMenuItem;
            ...
            /// Sheep Menu Item: if another pet was downloaded, the icon and text of this item will change.
            /// </summary>
        static ToolStripMenuItem careForSheepItem;
            /// <summary>
        ...
        public ContextMenuStrip Create()
        {
            ...
            // Item: Care For Sheep.
            careForSheepItem = new ToolStripMenuItem();
            careForSheepItem.Text = "&Care for Sheep";
            careForSheepItem.Click += new EventHandler(Care_Click);
            careForSheepItem.Image = Resources.icon.ToBitmap();
            careForSheepItem.Font = new Font(newSheepMenuItem.Font, newSheepMenuItem.Font.Style | FontStyle.Bold);
            menu.Items.Add(careForSheepItem);
        ...
        {
#if PORTABLE
            if (isOptionLoaded || isCareLoaded)
            {
        ...
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="System.EventArgs"/> instance containing the event data.</param>
        void Care_Click(object sender, EventArgs e)
        {
#if PORTABLE
            if (isAboutLoaded || isOptionLoaded)
            {

            }
            else if (!isCareLoaded)
            {
                isCareLoaded = true;
                Program.OpenCareDialog();
                isCareLoaded = false;
            }
#else
            OpenOptionWindow("xamlesheep://options");
